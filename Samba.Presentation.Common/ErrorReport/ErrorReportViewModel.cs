    ...
    class ErrorReportViewModel : ObservableObject
    {
        ...
        public void SubmitError()
        {
            ...
            string queryString = string.Format("from={0}&emaila={1}&file={2}",
                Uri.EscapeDataString("supports@hattechpos.co.za"),
                Uri.EscapeDataString("POS Pro 5 Error Report"),
                Uri.EscapeDataString(tempFile));

            var c = new WebClient();
            var result = c.UploadFile("http://www.hattech.co.za" + queryString, "POST", tempFile);
