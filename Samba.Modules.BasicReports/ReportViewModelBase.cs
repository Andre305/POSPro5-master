...
namespace Samba.Modules.BasicReports
{
    public abstract class ReportViewModelBase : ObservableObject
    {
        ...
        public string Background { get { return Selected ? "#FFCC0002" : "#808080"; } }
        public string Foreground { get { return Selected ? "White" : "White"; } }
        ...
        {
            var fnPDF = AskFileName("Report", ".pdf");
            if (!string.IsNullOrEmpty(fnPDF))
            {
                try
                {
                    string fnXPS = fnPDF.Replace(".pdf", ".xps");
                    SaveAsXps(fnXPS, Document);
                    XpsToPdf(fnPDF, fnXPS);
                    DeleteFile(fnXPS);
                }
                catch (Exception e)
                {
        ...
        public static void XpsToPdf(string pathPDF, string pathXPS)
        {
            var pdfXpsDoc = PdfSharp.Xps.XpsModel.XpsDocument.Open(pathXPS);
            PdfSharp.Xps.XpsConverter.Convert(pdfXpsDoc, pathPDF, 0);
            pdfXpsDoc.Close();
        }
        ...
        public void AddDefaultReportHeader(SimpleReport report, WorkPeriod workPeriod, string caption)
        {
            ...
            report.AddHeader(!string.IsNullOrEmpty(userInfo) ? userInfo : " POS Pro 5");
        ...
        public void AddDefaultReportFooter(FlowDocument report)
        {
            ...
            p.Inlines.Add(new Run("Powered by HATTECH") { FontWeight = FontWeights.Bold });
            report.Blocks.Add(p);
        }
    }
}
